...
const UIManager = {
    ...
    renderLayout(pageTitle) {
        ...
        const sidebarHtml = `
            <aside id="sidebar" class="sidebar flex flex-col h-screen fixed lg:static bg-white z-50">
                <div class="p-6 border-b">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 grad-primary rounded-xl flex items-center justify-center shadow-lg shadow-blue-200">
                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <div>
                            <h1 class="font-bold text-slate-900 tracking-tight">Emerge Social</h1>
                ...
                <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                    ${this.menuItems.map(item => `
                        <a href="${item.link}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${currentPath === item.link ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}">
                            <svg class="w-5 h-5 ${currentPath === item.link ? 'text-blue-600' : 'text-slate-400'}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-4 border-t">
                    <button onclick="AuthManager.signOut()" class="flex items-center gap-3 px-4 py-3 text-sm font-medium text-red-500 hover:bg-red-50 rounded-xl transition-all w-full group">
            ...
            <header class="bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-slate-100">
                <div class="px-4 lg:px-8 py-4 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <h2 class="text-xl lg:text-2xl font-bold text-slate-900 leading-none">${pageTitle}</h2>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="hidden md:flex flex-col items-end">
                            <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Aujourd'hui</span>
                            <span class="text-sm font-bold text-slate-700">${new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                        </div>
                    </div>
                </div>
            </header>
        ...
        mainContent.innerHTML = headerHtml + '<div id="page-content" class="p-4 lg:p-8 animate-fade-in"></div>';
